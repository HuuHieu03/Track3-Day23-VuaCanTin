...
class AgentState(TypedDict, total=False):
    """LangGraph state with explicit overwrite and append-only fields."""
    ...
    # Overwrite fields used as routing gates / single-value workflow outputs.
    # These deliberately do NOT use reducers: each node should replace the previous value.
    evaluation_result: str
    pending_question: str | None
    proposed_action: str | None
    approval: dict[str, Any] | None

    # Append-only audit/history fields. LangGraph merges updates using operator.add.
